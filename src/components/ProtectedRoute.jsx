...
const ProtectedRoute = ({ children, allowedRoles = [] }) => {
  ...
  const closeMenu = () => {
    ...
    if (menu && menu.classList.contains("show")) {
      ...
      menu.style.height = ""; // reset bootstrap inline height
    }
  };
    ...
    <div className={`site ${isLoggedIn && user_role === "admin" ? 'inner dashboard' : 'inner'} ${cart.length > 0 && 'cart'}`}>
      ...
      {((user_role === "user" && location.pathname !== "/bill") || user_role === "admin") && <header className="site-header">
        <div className='site-header-top'>
          {user_role === "user" && <div className='navbar navbar-expand-lg d-flex gap-2 justify-content-start' onClick={backBtn}>{location.pathname === "/" ? <div>&nbsp;</div> : <><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="m313-440 224 224-57 56-320-320 320-320 57 56-224 224h487v80H313Z" /></svg>Back</>}</div>}
        ...
        {(user_role === "admin" && location.pathname !== '/order-details') && <ul className="navbar-nav ms-auto">
