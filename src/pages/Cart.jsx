                            ...
                            {cart.map((item, index) => <div key={index} className="cart-item">
                                ...
                                <div className="cart-brow">
                                    <div className="input-group">
                                        <div className="pprice">{priceDisplay(parseInt(item.price)).replace("₹", "")}</div>
                                        <div className="opt">
                                            <button className="minus-item" onClick={() => decrement(item.product_code)}><i className="fa-solid fa-minus"></i></button>
                                            <div className="qty">{getQuantity(parseInt(item.product_code))}</div>
                                            <button className="plus-item" onClick={() => increment(item.product_code)}><i className="fa-solid fa-plus"></i></button>
                                        </div>
                                ...
                                <div>
                                    <div>
                                        ...
                                        {cart.map((item, index) => <div key={index} className="cart-item">
                                            ...
                                            <div className="cart-brow">
                                                <div className='pcode'>{`# ${item.product_code}`}</div>
                                                <div className="input-group alt">
                                                    <div>{getQuantity(parseInt(item.product_code))}</div>
                                                    <div>&times;</div>
                                                    <div>{priceDisplay(parseInt(item.price)).replace("₹", "")}</div>
                                                    <div>=</div>
                                                    <div>{getCartItemsAmount(item).replace("₹", "")}</div>
                                                </div>
                                            </div>
